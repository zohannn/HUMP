#include "../include/amplinterface.hpp"
...
#include "getstub.h"
  ...
  AmplInterface::AmplInterface(const SmartPtr<const Journalist>& jnlst,
  ...
  {
    DBG_START_METH("AmplInterface::AmplInterface",
                   dbg_verbosity);
    ...
    *fint_nerror = 0;
    ...
    ASSERT_EXCEPTION(allow_discrete || (nbv == 0 && niv == 0 && nlvbi == 0 && nlvci == 0 && nlvoi == 0),
                     ...
                     "Either remove the integer variables, or change the flag in the constructor of AmplInterface"
                    );
  ...
  {
    if (hesset_called_) {
      jnlst_->Printf(J_ERROR, J_MAIN, "Internal error: AmplInterface::set_active_objective called after AmplInterface::call_hesset.\n");
      THROW_EXCEPTION(INVALID_TNLP, "Internal error: AmplInterface::set_active_objective called after AmplInterface::call_hesset.");
    }
    ASL_pfgh* asl = asl_;
    obj_no = in_obj_no;
  ...
  void AmplInterface::call_hesset()
  {
    if (hesset_called_) {
      jnlst_->Printf(J_ERROR, J_MAIN, "Internal error: AmplInterface::call_hesset is called twice.\n");
      THROW_EXCEPTION(INVALID_TNLP, "Internal error: AmplInterface::call_hesset is called twice.");
    }
    ...
    if (n_obj == 0) {
      ...
      if (n_obj>1 && !set_active_objective_called_) {
        jnlst_->Printf(J_ERROR, J_MAIN,
                       "There is more than one objective function in the AMPL model, but AmplInterface::set_active_objective has not been called.\n");
        THROW_EXCEPTION(INVALID_TNLP, "There is more than one objective function in the AMPL model, but AmplInterface::set_active_objective has not been called");
      }
  ...
  bool AmplInterface::get_nlp_info(Index& n, Index& m, Index& nnz_jac_g,
  ...
  bool AmplInterface::get_var_con_metadata(Index n,
  ...
  bool AmplInterface::get_bounds_info(Index n, Number* x_l, Number* x_u, Index m, Number* g_l, Number* g_u)
  {
    ASL_pfgh* asl = asl_;
    DBG_ASSERT(asl_);
  ...
  bool AmplInterface::get_constraints_linearity(Index n,
  ...
  bool AmplInterface::get_starting_point(Index n, bool init_x, Number* x, bool init_z, Number* z_L, Number* z_U, Index m, bool init_lambda, Number* lambda)
  {
    ASL_pfgh* asl = asl_;
    DBG_ASSERT(asl_);
    DBG_ASSERT(n == n_var);
  ...
  bool AmplInterface::eval_f(Index n, const Number* x, bool new_x, Number& obj_value)
  {
    DBG_START_METH("AmplInterface::eval_f",
                   dbg_verbosity);
  ...
  bool AmplInterface::eval_grad_f(Index n, const Number* x, bool new_x, Number* grad_f)
  {
    DBG_START_METH("AmplInterface::eval_grad_f",
                   dbg_verbosity);
    ASL_pfgh* asl = asl_;
    DBG_ASSERT(asl_);
  ...
  bool AmplInterface::eval_g(Index n, const Number* x, bool new_x, Index m, Number* g)
  {
    DBG_START_METH("AmplInterface::eval_g", dbg_verbosity);
  ...
  bool AmplInterface::eval_jac_g(Index n, const Number* x, bool new_x,
                            Index m, Index nele_jac, Index* iRow,
                            Index *jCol, Number* values)
  {
    DBG_START_METH("AmplInterface::eval_jac_g",
                   dbg_verbosity);
    ASL_pfgh* asl = asl_;
    DBG_ASSERT(asl_);
    DBG_ASSERT(n == n_var);
  ...
  bool AmplInterface::eval_h(Index n, const Number* x, bool new_x,
                        Number obj_factor, Index m, const Number* lambda,
                        bool new_lambda, Index nele_hess, Index* iRow,
                        Index* jCol, Number* values)
  {
    DBG_START_METH("AmplInterface::eval_h",
                   dbg_verbosity);
    ASL_pfgh* asl = asl_;
    DBG_ASSERT(asl_);
    DBG_ASSERT(n == n_var);
  ...
  void AmplInterface::finalize_solution(SolverReturn status,
   ...
   * @return
   */
  SolverReturn AmplInterface::get_status(){
  ...
  /**
   * @brief AmplInterface::get_solutions
   * @param x
   * @param z_L
   * @param z_U
   * @param g
  ...
  void AmplInterface::get_solutions(std::vector<Number> &x, std::vector<Number> &z_L, std::vector<Number> &z_U, std::vector<Number> &lambda, Number &obj){
      ...
      for (int i =0; i < num_vars; ++i ){
          ...
          z_L.push_back(z_L_sol_[i]);
          z_U.push_back(z_U_sol_[i]);
      }
  ...
  {
    DBG_START_METH("AmplInterface::internal_objval",
                   dbg_verbosity);
    ASL_pfgh* asl = asl_;
    DBG_ASSERT(asl_);
  ...
  bool AmplInterface::internal_conval(const Number* x, Index m, Number* g)
  {
    DBG_START_METH("AmplInterface::internal_conval",
                   dbg_verbosity);
    ASL_pfgh* asl = asl_;
    DBG_ASSERT(asl_);
    DBG_ASSERT(m == n_con);
  ...
  bool AmplInterface::apply_new_x(bool new_x, Index n, const Number* x)
  {
    DBG_START_METH("AmplInterface::apply_new_x",
                   dbg_verbosity);
  ...
  void AmplInterface::write_solution_file(const std::string& message) const
  {
    ASL_pfgh* asl = asl_;
    DBG_ASSERT(asl);
  ...
  void AmplInterface::get_discrete_info(Index& nlvb_,
                                   Index& nlvbi_,
                                   Index& nlvc_,
                                   Index& nlvci_,
                                   Index& nlvo_,
  ...
  bool AmplInterface::get_scaling_parameters(Number& obj_scaling,
  ...
  Index AmplInterface::get_number_of_nonlinear_variables()
  {
    ASL_pfgh* asl = asl_;
    return Max(nlvo,nlvc);
  }

  bool AmplInterface::get_list_of_nonlinear_variables(Index num_nonlin_vars,
  ...
  /*
  char*
  AmplInterface::get_options(const SmartPtr<OptionsList>& options,
  ...
  */
  ...
  bool AmplInterface::nerror_ok(void* nerror)
  {
    DBG_START_METH("AmplInterface::nerror_ok",
                   dbg_verbosity);
